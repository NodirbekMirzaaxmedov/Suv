...
def all_user_products(search,page,limit,db,branch_id):
    products = db.query(User_products)
    if branch_id > 0:
        products = products.filter(User_products.branch_id == branch_id)
    if search:
        search_formatted = "%{}%".format(search)
        search_filter = products.filter(User_products.name.like(search_formatted))
    else:
        search_filter = User_products.id > 0
    products = products.filter(search_filter).order_by(User_products.name.asc())
