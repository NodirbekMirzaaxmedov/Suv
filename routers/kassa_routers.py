...
from functions.kassa_func import all_kassa_r, create_kassa_r, delete_kassa_e, update_kassa_rr
...
def update_kassas(this_kassa: UpdateKassa,db: Session = Depends(database),current_user: CreateUser = Depends(get_current_user)):
    role_verification(user=current_user)
    update_kassa_rr(this_kassa,db,current_user)
    raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")

@kassas_router.delete("/delete_kassas")
def delete_kassa(id: int, db: Session = Depends(database),
                current_user: CreateUser = Depends(get_current_user)):
    role_verification(user=current_user)
    delete_kassa_e(id, db)
    raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")
